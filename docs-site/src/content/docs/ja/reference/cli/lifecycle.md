...
明示的な外部アップグレードの後は `ocx service restart --if-needed --json` を使用します。選択されたパッケージが起動時のパッケージ ID と一致する場合は PID を維持し、インストールが変わった場合はサービスマネージャー経由で 1 回だけ再起動します。サービスが存在しない、または意図的に停止されている場合は明示的にスキップし、所有権、置換先、サービスマネージャー、ヘルス、準備完了状態を検証できない場合はスタンドアロンプロキシを起動せず失敗します。
