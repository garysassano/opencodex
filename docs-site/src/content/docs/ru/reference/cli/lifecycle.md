...
После явного внешнего обновления используйте `ocx service restart --if-needed --json`. Команда сохраняет PID, если выбранный пакет совпадает с идентификатором пакета при запуске, и выполняет ровно один перезапуск через менеджер службы, если установка изменилась. Отсутствующая или намеренно остановленная служба явно пропускается; если нельзя проверить владельца, замену, менеджер службы, health или readiness, команда завершается с ошибкой и не запускает отдельный прокси.
